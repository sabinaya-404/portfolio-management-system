...
<section class="dashboard-card" aria-labelledby="ipo-news-heading">
    ...
    <?php else: ?>
        <?php foreach ($entries as $entry): ?>
            <article class="dashboard-card" style="margin-bottom: 16px;">
                <div class="card-header"><div><span class="status-badge"><?= htmlspecialchars(strtoupper($entry["type"]), ENT_QUOTES, "UTF-8") ?></span><h3><?= htmlspecialchars($entry["title"], ENT_QUOTES, "UTF-8") ?></h3></div><time datetime="<?= htmlspecialchars($entry["publication_date"], ENT_QUOTES, "UTF-8") ?>"><?= htmlspecialchars(date("M j, Y", strtotime($entry["publication_date"])), ENT_QUOTES, "UTF-8") ?></time></div>
                <p><?= nl2br(htmlspecialchars($entry["content"], ENT_QUOTES, "UTF-8")) ?></p>
                <?php if (!empty($entry["source_url"])): ?><p style="margin-top: 12px;"><a class="view-link" href="<?= htmlspecialchars($entry["source_url"], ENT_QUOTES, "UTF-8") ?>" target="_blank" rel="noopener noreferrer">View source</a></p><?php endif; ?>
            </article>
        <?php endforeach; ?>
    <?php endif; ?>
</section>
